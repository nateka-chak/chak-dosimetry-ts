...
export default function DashboardPage() {
  ...
  const { hospital } = useParams(); // ✅ from route: /hospitals/[hospital]/dashboard

  useEffect(() => {
    const fetchShipments = async () => {
      try {
        let url = `${API_BASE_URL}/api/hospital-shipments`;

        // ✅ API expects ?hospital=
        if (hospital) {
          url += `?hospital=${encodeURIComponent(hospital.toString())}`;
        }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 px-6 py-8 space-y-10">
      ...
      {/* Status Timeline for the first shipment */}
      {shipments.length > 0 && (
